 ...
 * tasks would be generated by the LEVEL_ZERO engine when a lack of data for future tasks is detected, in which
 ...
 */
typedef parsec_task_t parsec_LEVEL_ZERO_d2h_task_t;

static int
hook_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                       parsec_LEVEL_ZERO_d2h_task_t* this_task )
{
...
static int
affinity_of_LEVEL_ZERO_d2h_task( parsec_LEVEL_ZERO_d2h_task_t* this_task,
...
static void
iterate_successors_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                                     const parsec_LEVEL_ZERO_d2h_task_t* this_task,
...
static void
iterate_predecessors_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                                       const parsec_LEVEL_ZERO_d2h_task_t* this_task,
                                       uint32_t action_mask,
                                       parsec_ontask_function_t * ontask, void *ontask_arg )
{
...
static int
release_deps_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                               parsec_LEVEL_ZERO_d2h_task_t* this_task,
                               uint32_t action_mask,
...
static int
data_lookup_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                              parsec_LEVEL_ZERO_d2h_task_t* this_task )
{
    (void)es; (void)this_task;
    return PARSEC_SUCCESS;
...
static int
complete_hook_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t* es,
                                parsec_LEVEL_ZERO_d2h_task_t* this_task )
{
    (void)es; (void)this_task;
    return PARSEC_SUCCESS;
...
release_task_of_LEVEL_ZERO_d2h_task(parsec_execution_stream_t* es,
                              parsec_LEVEL_ZERO_d2h_task_t* this_task )
{
    (void)es; (void)this_task;
...
static int
datatype_lookup_of_LEVEL_ZERO_d2h_task( parsec_execution_stream_t * es,
                                  const parsec_LEVEL_ZERO_d2h_task_t* this_task,
...
static int32_t parsec_LEVEL_ZERO_d2h_counter = 0;
static uint64_t key_of_LEVEL_ZERO_d2h_task(const parsec_taskpool_t *tp,
                                     const parsec_assignment_t *assignments)
{
    (void)tp; (void)assignments;
    return (uint64_t)(1 + parsec_atomic_fetch_inc_int32(&parsec_LEVEL_ZERO_d2h_counter));
}

static parsec_data_t*
flow_of_LEVEL_ZERO_d2h_task_direct_access( const parsec_LEVEL_ZERO_d2h_task_t* this_task,
                                     const parsec_assignment_t *assignments )
{
...
static const __parsec_chore_t __LEVEL_ZERO_d2h_task_chores[] = {
    {.type = PARSEC_DEV_LEVEL_ZERO,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) hook_of_LEVEL_ZERO_d2h_task},
    {.type = PARSEC_DEV_NONE,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) NULL},   /* End marker */
};

static const parsec_flow_t flow_of_LEVEL_ZERO_d2h_task;
static const parsec_dep_t flow_of_LEVEL_ZERO_d2h_task_dep = {
    .cond = NULL,
    .ctl_gather_nb = NULL,
    .task_class_id = -1,
    .direct_data = (parsec_data_lookup_func_t)flow_of_LEVEL_ZERO_d2h_task_direct_access,
    .dep_index = 1,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_LEVEL_ZERO_d2h_task,
};
...
static const parsec_flow_t flow_of_LEVEL_ZERO_d2h_task = {
    ...
    .dep_in = {},
    .dep_out = {&flow_of_LEVEL_ZERO_d2h_task_dep}
};

static const parsec_symbol_t symb_LEVEL_ZERO_d2h_task_param = {
    .name = "unnamed",
    .min = NULL,
    .max = NULL,
    .context_index = 0,
...
int32_t parsec_LEVEL_ZERO_d2h_max_flows = 0;

static const parsec_task_class_t parsec_LEVEL_ZERO_d2h_task_class = {
    .name = "LEVEL_ZERO D2H data transfer",
    .task_class_id = 0,
    ...
    .nb_parameters = 1,
    .nb_locals = 0,
    .params = {&symb_LEVEL_ZERO_d2h_task_param},
    .locals = {NULL},
    .data_affinity = (parsec_data_ref_fn_t *) affinity_of_LEVEL_ZERO_d2h_task,
    .initial_data = (parsec_data_ref_fn_t *) NULL,
    .final_data = (parsec_data_ref_fn_t *) NULL,
    .priority = NULL,
    .in = {&flow_of_LEVEL_ZERO_d2h_task, NULL},
    .out = {&flow_of_LEVEL_ZERO_d2h_task, NULL},
    ...
    .make_key = (parsec_functionkey_fn_t *) key_of_LEVEL_ZERO_d2h_task,
    .fini = (parsec_hook_t *) NULL,
    .incarnations = __LEVEL_ZERO_d2h_task_chores,
    .find_deps = parsec_hash_find_deps,
    .iterate_successors = (parsec_traverse_function_t *) iterate_successors_of_LEVEL_ZERO_d2h_task,
    .iterate_predecessors = (parsec_traverse_function_t *) iterate_predecessors_of_LEVEL_ZERO_d2h_task,
    .release_deps = (parsec_release_deps_t *) release_deps_of_LEVEL_ZERO_d2h_task,
    .prepare_input = (parsec_hook_t *) data_lookup_of_LEVEL_ZERO_d2h_task,
    .prepare_output = (parsec_hook_t *) NULL,
    .get_datatype = (parsec_datatype_lookup_t *) datatype_lookup_of_LEVEL_ZERO_d2h_task,
    .complete_execution = (parsec_hook_t *) complete_hook_of_LEVEL_ZERO_d2h_task,
    .release_task = (parsec_hook_t *) release_task_of_LEVEL_ZERO_d2h_task,
                           ...
                           parsec_execution_stream_t *es)
{
    ...
    parsec_LEVEL_ZERO_d2h_task_t *d2h_task = NULL;
    ...
    while(nb_cleaned < parsec_LEVEL_ZERO_d2h_max_flows) {
        ...
        if( 0 == gpu_copy->readers ) {
            if( PARSEC_UNLIKELY(NULL == d2h_task) ) {  /* allocate on-demand */
                d2h_task = (parsec_LEVEL_ZERO_d2h_task_t*)parsec_thread_mempool_allocate(es->context_mempool);
            ...
            parsec_atomic_unlock( &gpu_copy->original->lock );
            PARSEC_DEBUG_VERBOSE(10, parsec_level_zero_output_stream,  "D2H[%s] task %p:\tdata %d -> %p [%p] readers %d",
    ...
    d2h_task->task_class      = &parsec_LEVEL_ZERO_d2h_task_class;
                             ...
                             parsec_execution_stream_t *es)
{
    ...
    parsec_LEVEL_ZERO_d2h_task_t* task = (parsec_LEVEL_ZERO_d2h_task_t*)gpu_task->ec;
    parsec_data_t* original;

    PARSEC_DEBUG_VERBOSE(10, parsec_level_zero_output_stream,  "D2H[%s] task %p: %d data transferred to host",
